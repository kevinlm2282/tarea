...
class reportePDF(object):
    ...
    def __init__(self, titulo, cabecera, datos, nombrePDF, clienteVent):
    ...
    def _encabezadoPiePagina(canvas, archivoPDF):
        ...
        # Encabezado
        encabezadoNombre = Paragraph("Farmacia Santa Clara", estilos["Normal"])
        encabezadoNombre2 = Paragraph("Casa Matriz", estilos["Normal"])
        encabezadoNombre3 = Paragraph("Av Buenos Aires N 17", estilos["Normal"])
        
        anchura, altura = encabezadoNombre.wrap(archivoPDF.width, archivoPDF.topMargin)
        anchura2, altura2 = encabezadoNombre2.wrap(archivoPDF.width, archivoPDF.topMargin)
        anchura3, altura3 = encabezadoNombre3.wrap(archivoPDF.width, archivoPDF.topMargin)
        # anchura4, altura4 = encabezadoNombre4.wrap(archivoPDF.width, archivoPDF.topMargin)
        encabezadoNombre.drawOn(canvas, archivoPDF.leftMargin, 760)
        encabezadoNombre2.drawOn(canvas, archivoPDF.leftMargin, 750)
        encabezadoNombre3.drawOn(canvas, archivoPDF.leftMargin, 740)
        # encabezadoNombre4.drawOn(canvas, archivoPDF.leftMargin, 660)
    ...
    def convertirDatos(self):
        ...
        # alineacionCliente = ParagraphStyle(name)
    ...
    def Exportar(self):
        ...
        alineacionCosa = ParagraphStyle(name="centrar", alignment=TA_LEFT, fontSize=13,
                                          leading=50, textColor=black,
                                          parent=self.estilos["Heading1"])
        ...
        historia = []
        historia.append(Paragraph(self.clienteVent,alineacionCosa))
